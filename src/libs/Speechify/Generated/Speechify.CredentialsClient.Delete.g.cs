            ...
            try
            {
                ...
                else
                {
                    await global::Speechify.AutoSDKRequestOptionsSupport.OnAfterErrorAsync(
                            ...
                            // The credential is still referenced by one or more tools or agents. `used_by` names them so the caller can detach first.
                            if ((int)__response.StatusCode == 409)
                            {
                                string? __content_409 = null;
                                global::System.Exception? __exception_409 = null;
                                global::Speechify.CredentialInUseError? __value_409 = null;
                                try
                                {
                                    if (__effectiveReadResponseAsString)
                                    {
                                        __content_409 = await __response.Content.ReadAsStringAsync(__effectiveCancellationToken).ConfigureAwait(false);
                                        __value_409 = global::Speechify.CredentialInUseError.FromJson(__content_409, JsonSerializerContext);
                                    }
                                    else
                                    {
                                        __content_409 = await __response.Content.ReadAsStringAsync(__effectiveCancellationToken).ConfigureAwait(false);
                                ...
                                throw global::Speechify.ApiException<global::Speechify.CredentialInUseError>.Create(
                                    statusCode: __response.StatusCode,
